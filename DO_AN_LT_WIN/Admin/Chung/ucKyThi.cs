...
namespace DO_AN_LT_WIN.Admin.Chung
{
    public partial class ucKyThi : UserControl
    {
        ...
        {
            dtpTGBatDau.MinDate = new DateTime(2000, 1, 1);
            dtpTGBatDau.MaxDate = new DateTime(2026, 1, 1);

            dtpTGKetThuc.MinDate = new DateTime(2000, 1, 1);
            dtpTGKetThuc.MaxDate = new DateTime(2026, 1, 1);
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtMaKyThi.Text.Contains(" "))
                {
                    MessageBox.Show("Mã không có dấu space", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    txtMaKyThi.Text = "";
                    return;
                }
        ...
        private void UpdateTrangThai()
        {
            ...
            DateTime startTime = dtpTGBatDau.Value;
            DateTime endTime = dtpTGKetThuc.Value;

            if (endTime < now)
            {
                cbxTrangThai.Text = "Đã kết thúc";
            }
            else if (startTime <= now && endTime >= now)
            {
                cbxTrangThai.Text = "Đang diễn ra";
            }
            else if (startTime > now)
            {
                cbxTrangThai.Text = "Chưa diễn ra";
            }
        }
    }
}